...
must_haves:
  truths:
    ...
    - "Workflow creates GitHub Release with updater artifacts (latest.json, .tar.gz, .sig)"
...
<task type="auto">
  <name>Task 1: Create GitHub Actions release workflow for macOS ARM + Intel builds</name>
  <files>
    .github/workflows/release.yml
  </files>
  <action>
    Create `.github/workflows/release.yml` based on the Tauri v2 official GitHub Actions documentation and the Editor-Workshop proven setup.
    ...
    - What artifacts are produced (DMG, updater JSON, signatures)
  </action>
  <verify>
  ...
  <done>
    GitHub Actions workflow created that builds macOS app for ARM + Intel, signs with Apple certificates, generates updater artifacts, and publishes to GitHub Releases. Rust cache enabled for faster builds. All secrets documented.
  </done>
</task>
...
2. Workflow builds for both ARM and Intel macOS targets
3. All required GitHub Secrets are referenced in environment variables
4. Rust cache is configured for build performance
5. `scripts/bump-version.sh` updates all 3 version files
6. Workflow produces tagged releases with updater artifacts
</verification>
